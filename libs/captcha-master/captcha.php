...
function simple_php_captcha($config = array()) {
    ...
    if ( strpos($_SERVER['SCRIPT_FILENAME'], $_SERVER['DOCUMENT_ROOT']) ) {
        $image_src = substr(__FILE__, strlen( realpath($_SERVER['DOCUMENT_ROOT']) )) . '?_CAPTCHA[' . $captcha_config['location'] . ']&amp;t=' . urlencode(microtime());
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
    } else {
        $_SERVER['WEB_ROOT'] = str_replace($_SERVER['SCRIPT_NAME'], '', $_SERVER['SCRIPT_FILENAME']);
        $image_src = substr(__FILE__, strlen( realpath($_SERVER['WEB_ROOT']) )) . '?_CAPTCHA[' . $captcha_config['location'] . ']&amp;t=' . urlencode(microtime());
        $image_src = '/' . ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
    }

    $_SESSION['_CAPTCHA' . $captcha_config['location']]['config'] = serialize($captcha_config);
...
if( isset($_GET['_CAPTCHA']) ) {
  $captchaId = array_keys( $_GET['_CAPTCHA'] );

    session_start();

    $captcha_config = unserialize($_SESSION['_CAPTCHA' . $captchaId[0]]['config']);
    if( !$captcha_config ) exit();

    unset($_SESSION['_CAPTCHA' . $captchaId[0]]);
